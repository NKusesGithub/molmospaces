...
# Build/runtime caches that are NOT assets but behave like them: big, slow to
# regenerate, and — until these mounts existed — written only into the
# container's writable layer, so `docker rm` threw them away and `docker commit`
# baked them permanently into the image instead (that's most of why
# :with-molmospaces is 55GB against the 24GB base image).
#   pip   — wheels built from source in here (evdev, IsaacLab deps) cost the most
#           time to recreate; hoisting it also stops the next commit re-baking it.
#   ov    — Omniverse shader + texture cache. Losing it doesn't break anything,
#           it just makes the first Isaac Sim launch after a recreate crawl while
#           every shader recompiles.
# Both live outside the git repo dir, same reasoning as MOLMO_CACHE_PATH.
...
mkdir -p "$MOLMO_CACHE_PATH" "$PIP_CACHE_PATH" "$OV_CACHE_PATH"
sudo chmod -R a+rwX "$MOLMO_CACHE_PATH" "$PIP_CACHE_PATH" "$OV_CACHE_PATH"
...
echo "Build caches ready at $PIP_CACHE_PATH and $OV_CACHE_PATH (same reason: Docker would"
echo "otherwise auto-create these as root and the container's isaac-sim user couldn't write them)."
...
if ! docker ps -a --format '{{.Names}}' | grep -qx "$CONTAINER_NAME"; then
  ...
  docker run --name "$CONTAINER_NAME" --entrypoint bash -d --runtime=nvidia --gpus all \
    ...
    -v "$PIP_CACHE_PATH":/isaac-sim/.cache/pip \
    -v "$OV_CACHE_PATH":/isaac-sim/.cache/ov \
    "${extra_mounts[@]}" \
    --network host \
